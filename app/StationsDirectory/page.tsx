...
export default async function StationsDirectoryPage({ searchParams }: PageProps) {
  ...
  return (
    <>
      ...
      {isSearchMode ? (
        <section className="bg-[var(--navy)] py-6 sm:py-8">
          <div className="page-container !py-0">
            ...
            />
            <h1 className="text-h2 mt-3 text-white">Station phone number search</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-200">
              Results for &ldquo;{searchQuery}&rdquo; — custody desk lines, main numbers, and addresses
              across {stations.length > 0 ? `${stations.length} listed stations` : 'England & Wales'}.
            </p>
            <p className="mt-2 text-xs text-slate-400">
              <Link href="/StationsDirectory" className="font-semibold text-[var(--gold)] no-underline hover:underline">
                Browse full directory
              </Link>
              {' · '}
              <Link href="/UpdateStation" className="font-semibold text-[var(--gold)] no-underline hover:underline">
                Report a number
              </Link>
            </p>
          </div>
      ...
      <div className="page-container">
        {!isSearchMode ? <StationsDataContributeCta variant="slim" className="mb-6" /> : null}
        ...
        {isSearchMode ? <StationsDataContributeCta variant="slim" className="mt-6" /> : null}
