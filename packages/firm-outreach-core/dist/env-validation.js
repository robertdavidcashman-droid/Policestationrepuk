...
function validateOutreachEnv(opts) {
    ...
    const sendingEnabled = !falsy(process.env.FIRM_OUTREACH_ENABLED) &&
    ...
    if ((opts?.requireWebhookSecret || opts?.forLiveSend) &&
    ...
    if (!psaFrom) {
        warnings.push('FIRM_OUTREACH_PSA_FROM_EMAIL unset — PSA prefers noreply@policestationagent.com and falls back to verified RepUK domain until that domain is verified on Resend');
    }
