...
describe('recoverEnrichPool', () => {
  beforeEach(() => {
    mocks.getProspectsByIds.mockReset();
    mocks.listProspectIdsByRecordStatus.mockReset();
    mocks.saveProspect.mockReset();
  });

  it('retires exhausted discovered rows to no_email via batched lookup', async () => {
    ...
    expect(mocks.getProspectsByIds).toHaveBeenCalledWith(['p1']);
