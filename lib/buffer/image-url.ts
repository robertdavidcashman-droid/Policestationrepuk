...
/** Legacy cross-site fallback — last resort only. */
...
export type GbpFeedId = (typeof GBP_FEED_IDS)[number];
...
export function googleBusinessFeedFallbackUrl(
  feedId: string,
...
/** Self-hosted GBP JPEG defaults on policestationrepuk.org. */
export function buildGoogleBusinessFeedFallbacks(siteUrl: string = SITE_URL): Record<string, string> {
  return Object.fromEntries(GBP_FEED_IDS.map((id) => [id, googleBusinessFeedFallbackUrl(id, siteUrl)]));
}

export const GOOGLE_BUSINESS_FEED_FALLBACKS = buildGoogleBusinessFeedFallbacks();
...
function publicFilePathFromSiteUrl(url: string, siteUrl: string = SITE_URL): string | null {
  try {
...
/** Validate images shipped in public/ without a network round-trip (dev, CI, pre-deploy). */
...
export function googleBusinessImageCandidates(
  ...
  if (feedId && GOOGLE_BUSINESS_FEED_FALLBACKS[feedId]) {
    out.push(GOOGLE_BUSINESS_FEED_FALLBACKS[feedId]!);
  } else if (isLocal) {
    out.push(`${base}/social-preview.jpg`);
  }

  if (isExternal && feedId && GOOGLE_BUSINESS_FEED_FALLBACKS[feedId]) {
    const hosted = GOOGLE_BUSINESS_FEED_FALLBACKS[feedId]!;
...
async function verifyGoogleBusinessMagicBytes(
  url: string,
...
/** Probe image for Google Business — JPEG/PNG only, magic bytes, ≤ 5MB. */
export async function probeGoogleBusinessImageUrl(
  ...
  const local = probeLocalPublicImageUrl(trimmed);
...
export async function assertBufferPostImageReady(
  ...
  if (options?.channelService === 'googlebusiness') {
    const resolved = await resolveGoogleBusinessImageUrl(trimmed, fetchFn, SITE_URL, options.feedId);
    if (!resolved) {
